...
nextflow run labsyspharm/brca-profiling -r main --in genesets
```
(where `main` specifies the branch in this GitHub repository.)
...
```
nextflow run labsyspharm/brca-profiling -r main --in my_signatures --out /path/to/results
```
...
nextflow run labsyspharm/brca-profiling -r main --in my_signatures --cell-lines cl.txt
```
...
```
module load java
nextflow run labsyspharm/brca-profiling -r main --in genesets -profile O2
```
